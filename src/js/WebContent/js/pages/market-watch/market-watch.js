...
import { createSukukFilters } from "./market-watch.filters.js";
...
function getMobileFieldLabel(column, config) {
	  // Directly resolves standard column text for the 4 static columns
	  return cleanLabel(
...
function getMobileDetailColumns(config, view, visibleGroups) {
	  // Directly pipes your visible columns out to the card renderer loop
	  return getMobileColumns(config, view, visibleGroups);
	}
...
function renderMarketWatchCard(row, context, config, visibleGroups) {
	  ...
	  const fields = getMobileDetailColumns(
	    ...
	    fullWidth: column.key === "companyName",

	    numeric: column.key !== "companyName",
	  }));
...
export function initMarketWatch(root = document) {
  ...
  const cards = createDataCards({
    ...
    errorMessage: config.labels?.loadError || "Unable to load theoretical data.",
  });
  ...
  const results = resultCountElement
    ? createDataResults({
        ...
        labels: {
          ...
          error: config.labels?.loadError || "Unable to load theoretical data.",
        },
      })
    : null;
