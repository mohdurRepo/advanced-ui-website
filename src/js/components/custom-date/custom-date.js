...
/* ==========================================================================
   Stale Enhancement Cleanup
   ========================================================================== */

/**
 * Removes generated enhancement markup from a component that does not have a
 * live CustomDate instance.
 *
 * This can happen when already-enhanced DOM is serialized, cloned, returned
 * through AJAX, or inserted into the document again. Generated interface
 * markup must not be reused because a fresh CustomDate instance owns its own
 * event listeners, observers, and generated interface.
 */
function cleanupStaleEnhancement(component) {
  if (!component || instances.has(component)) {
    return;
  }

  /*
   * Remove only generated interfaces that are direct children of this
   * component. This avoids touching nested custom-date components.
   */
  Array.from(component.children).forEach((child) => {
    if (child.matches?.(SELECTORS.interface)) {
      child.remove();
    }
  });

  /*
   * A copied enhanced component may also contain a hidden fallback. Restore it
   * before the new instance captures its original state. The constructor hides
   * it again after successful enhancement.
   */
  const fallbackElement = component.querySelector(SELECTORS.fallback);
  ...
  /*
   * Remove transient enhancement state copied from a previous instance.
   */
  ...
  component.removeAttribute(DATA_ATTRIBUTES.activeBoundary);
}
...
export class CustomDate {
  ...
  static getOrCreateInstance(component, options = {}) {
    ...
    /*
     * AJAX or cloned markup may contain an interface generated by an earlier
     * CustomDate instance. Remove stale generated state before creating the
     * live instance for this DOM element.
     */
    cleanupStaleEnhancement(component);
  ...
  constructor(component, options = {}) {
    ...
    /*
     * Keep a reference to the server-rendered/native fallback. The fallback is
     * visible before enhancement and hidden only after the enhanced interface
     * has been created successfully.
     */
    this.fallbackElement = this.component.querySelector(SELECTORS.fallback);
    ...
    this.original = Object.freeze({
      ...
      fallbackHidden: this.fallbackElement?.getAttribute("hidden") ?? null,
    ...
    /*
     * Hide the fallback explicitly after enhancement succeeds. This prevents
     * both the fallback trigger and enhanced trigger from appearing when a
     * component is inserted after page load.
     */
  ...
  focusActivePickerOption() {
    ...
    /*
     * The year picker may contain more than 100 years.
     *
     * Its DOM begins with the oldest year, so without explicitly scrolling the
     * selected option into view the picker appears to open around 19xx even
     * though the current year is correctly selected.
     */
    if (record.element.hasAttribute(DATA_ATTRIBUTES.year)) {
      try {
  ...
  destroy() {
    ...
    /*
     * Remove the interface generated by this CustomDate instance.
     */
    this.interfaceElement.remove();

    /*
     * Restore the fallback to exactly the state it had before enhancement.
     *
     * If it originally had hidden="", restore hidden.
     * Otherwise remove hidden so the native/fallback UI becomes available
     * again after destruction.
     */
    restoreAttribute(
      this.fallbackElement,
      "hidden",
      this.original.fallbackHidden,
    );
