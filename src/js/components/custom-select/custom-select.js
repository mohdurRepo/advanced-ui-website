 ...
 * markup belongs to the instance that created it and must not be reused by a
 * fresh CustomSelect instance.
 */
function cleanupStaleEnhancement(component) {
  if (!component || instances.has(component)) return;

  const staleInterfaces = Array.from(component.children).filter((child) =>
    child.matches?.(SELECTORS.interface),
  );

  const hasStaleEnhancement =
    staleInterfaces.length > 0 ||
    component.classList.contains(CLASS_NAMES.enhanced);

  if (!hasStaleEnhancement) return;

  /*
   * Remove only generated interfaces that are direct children of this
   * component. This avoids touching nested custom-select components.
   */
  staleInterfaces.forEach((interfaceElement) => {
    interfaceElement.remove();
  });

  /*
   * A copied enhanced component may contain a hidden fallback. Restore it
  ...
  /*
   * Remove transient state copied from the previous enhancement. Persistent
   * application state such as disabled/loading and the native multiple mode is
...
export class CustomSelect {
  ...
  static getOrCreateInstance(component, options = {}) {
     ...
     * CustomSelect instance. Remove stale generated state before constructing
     * the live instance for this DOM element.
     */
  ...
  constructor(component, options = {}) {
    ...
    /*
     * Keep a reference to the server-rendered/native fallback. It remains
     * usable until enhancement succeeds and is restored when this instance is
     * destroyed.
     */
    ...
    /*
     * Hide the native fallback explicitly only after the enhanced interface has
     * been created successfully. This prevents the fallback and generated
     * control from appearing together after AJAX insertion.
     */
  ...
  destroy() {
    ...
    /*
     * Restore the fallback to exactly the hidden state it had before this
     * instance enhanced the component.
     */
    restoreAttribute(
